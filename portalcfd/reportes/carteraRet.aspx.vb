Imports System.Data
Partial Class portalcfd_reportes_carteraRet
    ...
    Private Sub MuestraReporte()
        '
        Dim ObjData As New DataControl
        ds = ObjData.FillDataSet("exec pMisInformes @cmd=16, @clienteid='" & clienteid.SelectedValue.ToString & "'")
    ...
    Protected Sub reporteGrid_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles reporteGrid.ItemDataBound
        Select Case e.Item.ItemType
            Case Telerik.Web.UI.GridItemType.Item, Telerik.Web.UI.GridItemType.AlternatingItem
                ...
                End If
                '
                e.Item.Cells(9).Text = FormatCurrency(e.Item.DataItem("importe") * 0.04, 2).ToString
                e.Item.Cells(9).HorizontalAlign = HorizontalAlign.Right
                '
                e.Item.Cells(10).Text = FormatCurrency(e.Item.DataItem("total") - (e.Item.DataItem("importe") * 0.04), 2).ToString
            ...
            Case Telerik.Web.UI.GridItemType.Footer
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    '
                    e.Item.Cells(9).Text = FormatCurrency(ds.Tables(0).Compute("sum(importe)*0.04", ""), 2).ToString
                    e.Item.Cells(9).HorizontalAlign = HorizontalAlign.Right
                    e.Item.Cells(9).Font.Bold = True
                    '
                    e.Item.Cells(10).Text = FormatCurrency(ds.Tables(0).Compute("sum(total)-sum(importe)*0.04", ""), 2).ToString
                    e.Item.Cells(10).HorizontalAlign = HorizontalAlign.Right
