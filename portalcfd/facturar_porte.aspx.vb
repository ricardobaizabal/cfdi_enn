...
Partial Public Class facturar_porte
...
#Region "Create Invoice"
        ...
        Else
            ...
            If folio > 0 Then
                If TimbradoFacturaxion() = True Then
                    ...
                    '
                    generarXmlDoc().Save(Server.MapPath("cfd_storage") & "\" & "link_" & serie.ToString & folio.ToString & "_timbrado.xml")
                    '
    ...
    Private Sub AsignaSerieFolio()
        ...
        If Not chkAduana.Checked Then
            SQLUpdate = "exec pCFD @cmd=17, @cfdid='" & Session("CFD").ToString & "', @tipodocumentoid='" & serieid.SelectedValue.ToString & "', @enviara='" & enviara.Text & "', @instrucciones='" & instrucciones.Text & "', @aduana='" & nombreaduana.Text & "', @fecha_pedimento='', @numero_pedimento='" & numeropedimento.Text & "', @tipocambio='" & tipocambio.Text & "', @formapagoId='" & formapagoid.SelectedValue.ToString & "', @tipopagoId='" & tipopagoid.SelectedValue.ToString & "', @numctapago='" & txtNumCtaPago.Text & "', @condicionesid='" & condicionesId.SelectedValue.ToString & "', @origen='" & txtOrigen.Text & "', @destino='" & txtDestino.Text & "', @remitente='" & txtRemitente.Text & "', @destinatario='" & txtDestinatario.Text & "', @rfc_origen='" & txtRFCRemitente.Text & "', @rfc_destinatario='" & txtRFCDestinatario.Text & "', @domicilio_origen='" & txtDomicilioRemitente.Text & "',  @domicilio_destino='" & txtDomicilioDestinatario.Text & "', @se_recogera='" & txtSeRecogera.Text & "', @se_entregara='" & txtEntregara.Text & "'"
        Else
    ...
    Private Function TimbradoFacturaxion() As Boolean
        ...
        Try
            ...
            DocParams.Save(Server.MapPath("cfd_storage") & "\" & "link_params_" & serie.ToString & folio.ToString & ".xml")

            DocXMLTimbrado.LoadXml(resultado)
            '
            '   Guarda XML del Timbre
            '
            DocXMLTimbrado.Save(Server.MapPath("cfd_storage") & "\" & "link_timbre_" & serie.ToString & folio.ToString & ".xml")
            '
...
#Region "Manejo de PDF"
    ...
    Private Function GeneraPDF(ByVal cfdid As Long) As Telerik.Reporting.Report
        ...
        Dim rfc_origen As String = ""
        Dim rfc_destinatario As String = ""
        Dim domicilio_origen As String = ""
        Dim domicilio_destino As String = ""
        Dim se_recogera As String = ""
        Dim se_entregara As String = ""
        ...
        Try
            ...
            If rs.Read Then
                ...
                origen = rs("origen")
                destino = rs("destino")
                remitente = rs("remitente")
                destinatario = rs("destinatario")
                rfc_origen = rs("rfc_origen")
                rfc_destinatario = rs("rfc_destinatario")
                domicilio_origen = rs("domicilio_origen")
                domicilio_destino = rs("domicilio_destino")
                se_recogera = rs("se_recogera")
                se_entregara = rs("se_entregara")
                '
            End If
            rs.Close()
            '
        ...
        Select Case tipoid
            Case 3, 6, 7      ' Recibo de honorarios y arrendamiento
                ...
                Return reporte
            Case Else
                Dim reporte As New Formatos.formato_cfdi_porte
                reporte.ReportParameters("plantillaId").Value = plantillaid
                reporte.ReportParameters("cfdiId").Value = cfdid
                Select Case tipoid
                    Case 1, 4, 7
                    ...
                    Case 5  ' Carta Porte
                        ...
                        reporte.ReportParameters("txtRemitente").Value = remitente
                        reporte.ReportParameters("txtDestinatario").Value = destinatario
                        reporte.ReportParameters("txtRFCRemitente").Value = rfc_origen
                        reporte.ReportParameters("txtRFCDest").Value = rfc_destinatario
                        reporte.ReportParameters("txtDomicilioRemitente").Value = domicilio_origen
                        reporte.ReportParameters("txtDomicilioDest").Value = domicilio_destino
                        reporte.ReportParameters("txtRecogera").Value = se_recogera
                        reporte.ReportParameters("txtEntregara").Value = se_entregara
